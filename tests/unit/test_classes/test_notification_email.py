...
class TestBackupNotificationDispatch:
    """The backup flow dispatches to both Discord and Email via _send_notification."""
    ...
    def _patched(self, already_sent=False):
        with patch("classes.bqckup.get_server_ip", return_value="1.2.3.4"), \
             patch("classes.bqckup.get_today", return_value="04-June-2026"), \
             patch("classes.bqckup.is_debug", return_value=False), \
             patch("classes.bqckup.NotificationLog") as MockNL, \
             patch("classes.bqckup.send_notification") as mock_discord, \
            ...
            yield mock_discord, mock_email

    def test_dispatches_to_email_and_discord(self):
        bq = self._bqckup()
        with self._patched(already_sent=False) as (mock_discord, mock_email):
            bq._send_notification(backup_name="site", title="Backup Failed")
    ...
    def test_skips_duplicate_notification(self):
        bq = self._bqckup()
        with self._patched(already_sent=True) as (mock_discord, mock_email):
            bq._send_notification(backup_name="site", title="Backup Failed")
